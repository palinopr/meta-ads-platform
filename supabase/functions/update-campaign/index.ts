...
serve(withSentryMonitoring('update-campaign', async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders })
  }

  try {
    console.log('update-campaign: Starting...')
    
    // Parse request body
    let updateData: UpdateCampaignData
    try {
    ...
    try {
      console.log('Updating campaign in Meta API with rate limiting...')
      ...
      if (updateData.daily_budget !== undefined) {
        metaUpdateData.daily_budget = updateData.daily_budget > 0 ? Math.round(updateData.daily_budget * 100) : null
      }
      if (updateData.lifetime_budget !== undefined) {
        metaUpdateData.lifetime_budget = updateData.lifetime_budget > 0 ? Math.round(updateData.lifetime_budget * 100) : null
      }
      ...
      if (!metaResponse.ok) {
        ...
        return new Response(
          JSON.stringify({ 
            error: errorData.error?.message || 'Failed to update campaign in Meta API',
            metaError: true
          }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      ...
      console.log('Campaign updated in Meta API:', metaResult)
      ...
      if (updateData.daily_budget !== undefined) updatedFields.daily_budget = updateData.daily_budget
      if (updateData.lifetime_budget !== undefined) updatedFields.lifetime_budget = updateData.lifetime_budget
      ...
      if (updateError) {
        ...
        return new Response(
          JSON.stringify({ 
            success: true,
            warning: 'Campaign updated in Meta but may not be saved locally. Please refresh to see changes.',
            campaign: { ...campaign, ...updatedFields }
          }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      console.error('Campaign update error:', fetchError)
      
      return new Response(
        JSON.stringify({ 
          error: `Failed to update campaign: ${fetchError.message}`,
          fetchError: true
        }),
    ...
    console.error('Unexpected error in update-campaign:', error)
