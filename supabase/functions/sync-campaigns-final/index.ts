...
serve(async (req) => {
  ...
  try {
    console.log('sync-campaigns-final: Starting...')
    ...
    // Transform campaigns for database - use correct column names
    const campaignsToInsert = (metaData.data || []).map((campaign: any) => ({
      campaign_id: campaign.id,
      ad_account_id: adAccount.id, // Correct column name
      ...
      // Note: No user_id column in campaigns table
    }))
    ...
    if (campaignsToInsert.length > 0) {
      ...
      savedCampaigns = insertedCampaigns || []
      console.log(`Successfully saved ${savedCampaigns.length} campaigns`)
    }
    ...
    console.error('Unexpected error in sync-campaigns-final:', error)
